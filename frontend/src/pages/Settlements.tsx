...
const Settlements = () => {
  ...
  // 获取结算记录
  const fetchSettlements = async (params?: { supplierName?: string; customerName?: string; billType?: string; paymentMethod?: string }) => {
    try {
      const response = await settlementApi.getSettlements(params);
      
      // 按照创建时间排序，最新的在前
      const sortedSettlements = response.data.sort((a, b) => {
        if (a.createdAt && b.createdAt) {
          return dayjs(b.createdAt).valueOf() - dayjs(a.createdAt).valueOf();
        } else {
          return dayjs(b.settlement_date).valueOf() - dayjs(a.settlement_date).valueOf();
        }
      });
  ...
  const handleSettlementSearch = async (values: any) => {
    try {
      ...
      const response = await settlementApi.getSettlements(params);
      let filteredSettlements = response.data;
      
      // 应用日期筛选（前端）
      if (values.dateRange && values.dateRange[0]) {
        filteredSettlements = filteredSettlements.filter(settlement => {
          return dayjs(settlement.settlement_date).isAfter(dayjs(values.dateRange[0]).startOf('day'));
        });
      }
      
      if (values.dateRange && values.dateRange[1]) {
        filteredSettlements = filteredSettlements.filter(settlement => {
          return dayjs(settlement.settlement_date).isBefore(dayjs(values.dateRange[1]).endOf('day'));
        });
      }
    ...
    <div>
      ...
      >
        ...
        <div style={{ marginBottom: 24, padding: 16, backgroundColor: '#f5f5f5', borderRadius: 8 }}>
          ...
          >
            <Column 
              title={billType === 'purchase' ? '入库单号' : '出库单号'} 
              dataIndex="id" 
              key="id" 
              ellipsis 
              width={200}
            />
            <Column 
              title={billType === 'purchase' ? '供应商' : '客户'} 
              key="supplier_customer" 
              render={(record) => {
                if (billType === 'purchase' && record.Supplier) {
                  return record.Supplier.name;
                } else if (billType === 'sale' && record.Customer) {
                  return record.Customer.name;
                }
                return '-';
              }} 
            ...
            />
            <Column 
              title={billType === 'purchase' ? '应付金额' : '应收金额'} 
